...
if( $IsWindows ) {
	function scoopUserModePostInstall {
		...
		'main' , 'extras' , 'nirsoft' , 'sysinternals' , 'versions' | % { if( ! ( git config --global --get-all safe.directory | sls $_ ) ) { git config --global --add safe.directory `"$env:ProgramFiles/scoop/buckets/$_`" } }
