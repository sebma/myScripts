...
if( $IsWindows ) {
	function scoopUserModePostInstall {
		...
		'main' , 'extras' , 'nirsoft' , 'sysinternals' , 'versions' | % { if( ! ( git config --get-all safe.directory | sls $_ ) ) { git config --add safe.directory `"$env:SCOOP_GLOBAL/buckets/$_`" } }
		git config --global --add safe.directory "$env:ProgramFiles/scoop/apps/scoop/current"
