...
if $isDebianLike;then
	...
	$sudo grep '^\s*Defaults:%sudo env_keep.*https_proxy' /etc/sudoers /etc/sudoers.d/* -q || echo 'Defaults:%sudo env_keep += "http_proxy https_proxy ftp_proxy all_proxy no_proxy"' | $sudo tee -a /etc/sudoers.d/proxy_env
	$sudo grep "^\s*Defaults:%$adminGroup env_keep.*https_proxy" /etc/sudoers /etc/sudoers.d/* -q || echo "Defaults:%$adminGroup env_keep += \"http_proxy https_proxy ftp_proxy all_proxy no_proxy\"" | $sudo tee -a /etc/sudoers.d/proxy_env
	test -s /etc/sudoers.d/proxy_env  && sudo chmod 440 /etc/sudoers.d/proxy_env
set +x
fi

if which snap &>/dev/null;then
	$sudo snap get system proxy.http >/dev/null  || $sudo snap set system proxy.http=$http_proxy
	$sudo snap get system proxy.https >/dev/null || $sudo snap set system proxy.https=$https_proxy
