...
if which omreport >/dev/null;then
		...
		stripesize=$(omreport storage vdisk controller=$controllerID vdisk=$latestVirtualDiskID | awk '/Stripe Element Size/{value=$(NF-1)$NF;printf tolower(value)}')		diskcachepolicy=$(omreport storage vdisk controller=$controllerID vdisk=$latestVirtualDiskID | awk '/Disk Cache Policy/{value=$NF}END{printf tolower(value)}')
		case $readpolicy in
				...
				*) echo "=> ERROR [$scriptName] : Unsupported read policy: <$readpolicy>." >&2;exit 7;;
		esac
		...
		case $writepolicy in
				...
				*) echo "=> ERROR [$scriptName] : Unsupported write policy = <$writepolicy>." >&2;exit 8;;
		esac
		...
		if ! omreport storage controller controller=$controllerID | grep Preserved.Cache.*Not.Applicable -q;then
				echo "=> ERROR [$scriptName] : There is Preserved Cache on the controller $controllerID." >&2
