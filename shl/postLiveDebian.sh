...
#permet le sudo pour lancer des applis graphiques
grep -q xhost.*root ~/.profile || echo xhost SI:localuser:root >> ~/.profile

dpkg -l console-data >/dev/null 2>&1 || $installPackages -y console-data
sudo loadkeys fr-latin9

dpkg -l command-not-found >/dev/null 2>&1 || $installPackages -y command-not-found
sudo update-command-not-found

$installPackages -y vim
grep -q syn ~/.vimrc || echo syntax on >> ~/.vimrc
...
alias burncdrw='cdrecord -v -dao driveropts=burnfree,noforcespeed fs=14M speed=12 gracetime=10 -eject'
alias burnclone='cdrecord -v -clone -raw driveropts=burnfree,noforcespeed fs=14M speed=16 gracetime=10 -eject -overburn'
alias burniso='cdrecord -v -dao driveropts=burnfree,noforcespeed fs=14M speed=24 gracetime=10 -eject -overburn'
...
test $distribVersion -gt 9 && universePackageList="alacarte aptitude system-config-lvm dconf-tools add-apt-key chromium-browser libcdio-utils abcde bashburn cclive w3m-img" || universePackageList="alacarte aptitude system-config-lvm add-apt-key chromium-browser libcdio-utils abcde cclive w3m-img"
