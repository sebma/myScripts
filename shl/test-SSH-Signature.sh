...
$(ssh-keyscan -t rsa,ed25519 $sftpServer 2>&1 | grep -v '^#')
...
ssh-keygen -f $HOME/.ssh/known_hosts -R $sftpServer

timeout 1s ssh -o StrictHostKeyChecking=accept-new $sftpServer : # Works since OpenSSH 7.6/Ubuntu 18.04
