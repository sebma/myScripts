...
#	$sudo smartctl -t short $diskDevice && sleep 60
	...
	echo
	$sudo smartctl -A $diskDevice | egrep -v "(Current_Pending_Sector|Offline_Uncorrectable).* 0$" | egrep "(Current_Pending_Sector|Offline_Uncorrectable)" | egrep --color=always " [0-9]+$"
	echo
