...
set +e
	sync
	set -e
	date +"Il est %T, lancement de grub-install pour le mode BIOS (dure environ 1 minute)..."
	...
	date +"Il est %T, lancement de grub-install pour le mode UEFI (dure environ 1 minute)..."
	$sudo bash -xc "time grub-install --target=x86_64-efi --boot-directory=$pendriveMountPoint/boot --modules=\"$grubFSModulName\" --removable --efi-directory=$pendriveMountPoint/boot/efi $pendriveDevice"
	sync
	$sudo parted $pendriveDevice set 1 boot on

	cd && $sudo umount -v "$pendriveMountPoint" && test -d "$pendriveMountPoint" && $sudo rmdir "$pendriveMountPoint"
