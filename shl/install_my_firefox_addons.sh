	...
	then
	  case $distribName in
			centos|redhat|ubuntu)
				test -d /usr/lib/firefox/browser/extensions && extensionDir=/usr/lib/firefox/browser/extensions || extensionDir=$(awk -F= '/LIBDIR=\//{print$2"/browser/extensions"}' $(which firefox))
				test -d $extensionDir || extensionDir=/opt/firefox/browser/extensions
				;;
			debian) extensionDir=/usr/lib/iceweasel/browser/extensions ;;
			*) echo "$yellowOnRed=> <$distribName> is not supported by this script for the time being.$normal" >&2; return 3;;
		esac
	else
		profileName=$(awk -F= '/Profile0/{found=1} /Profile[^0]/{found=0} /Path=/ && found==1 {print$2}' $HOME/.mozilla/firefox/profiles.ini)
		extensionDir=$HOME/.mozilla/firefox/$profileName/extensions
	fi
...
#	addonNumber[]=
	...
	firefoxAddonBaseURL=https://addons.mozilla.org/firefox/downloads/latest
...
#	for currentAddonName in $(echo "${!addonNumber[@]}"| tr ' ' '\n' | sort)
	...
	do
		...
		currentAddonURL=$firefoxAddonBaseURL/${addonNumber[$currentAddonName]}
		set +e
		...
		xmlstarlet validate --err $installRDFFileName || {
			...
			echo "=> ERROR: This <$installRDFFileName> file in an invalid xml file, proceeding to the next firefox extension ...$normal" >&2
			echo
			continue
		}
		...
		do
			xpathMinFirefoxVersionQuery="//Description[em:id=\"$fireFoxExtensionID\"]/em:minVersion/text()"
			xpathMaxFirefoxVersionQuery="//Description[em:id=\"$fireFoxExtensionID\"]/em:maxVersion/text()"
		...
		#Render <aardvark> extension compatible with firefox version currently installed
		toolbarButtonsExtensionID="{03B08592-E5B4-45ff-A0BE-C1D975458688}"
		if echo $extensionID | egrep -q "aardvark"
		then
			printf "=> Before, Firefox em:maxVersion = "
			$xpathCMD $xpathMaxFirefoxVersionQuery $installRDFFileName
			xmlstarlet edit --inplace --update $xpathMaxFirefoxVersionQuery -v 29.0 $installRDFFileName
			printf "=> After, Firefox em:maxVersion = "
			$xpathCMD $xpathMaxFirefoxVersionQuery $installRDFFileName
			zip -q9f "$xpiFile" $installRDFFileName
		fi


		minFirefoxVersionSupported=$($xpathCMD $xpathMinFirefoxVersionQuery $installRDFFileName | tr -d "[A-Za-z]" | sed "s/*/9/" | awk -F. '{print $1"."$2$3$4}')
		maxFirefoxVersionSupported=$($xpathCMD $xpathMaxFirefoxVersionQuery $installRDFFileName | tr -d "[A-Za-z]" | sed "s/*/9/" | awk -F. '{print $1"."$2$3$4}')
#		echo "=> maxFirefoxVersionSupported = $maxFirefoxVersionSupported"

#		if $isAdmin
#		then
			if echo $firefoxVersion | grep -q "^3\."
			then
				...
				then
					...
					echo "=> Verification de la compatibilite du module <$currentAddonName> avec Firefox v$firefoxVersion ..."
#					if [ $(echo $firefoxVersion \< $minFirefoxVersionSupported | bc -l) = 1 ] || [ $(echo $firefoxVersion \> $maxFirefoxVersionSupported | bc -l) = 1 ]
...
#				echo "=> La version de Firefox est > 3"
