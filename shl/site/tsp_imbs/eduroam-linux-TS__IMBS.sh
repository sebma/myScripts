...
function setup_environment {
  bf=""
  n=""
  ORGANISATION="Télécom SudParis & Institut Mines-Télécom Business School"
  URL="http://helpdesk.tem-tsp.eu"
  SUPPORT="helpdesk@imtbs-tsp.eu"
if [ ! -z "$DISPLAY" ] ; then
...
function confirm_exit {
  if [ ! -z $KDIALOG ] ; then
     if $KDIALOG --yesno "Voulez vous quitter la page ?"  ; then
     exit 1
     fi
  fi
  if [ ! -z $ZENITY ] ; then
     if $ZENITY --question --text="Voulez vous quitter la page ?" 2>/dev/null ; then
        exit 1
     fi
  fi
}
...
function user_cred {
  ...
  if ! USER_NAME=`prompt_nonempty_string 1 "entrez votre identifiant d'utilisateur"` ; then
    exit 1
  fi

  while [ "$PASSWORD" != "$PASSWORD1" ]
  do
    if ! PASSWORD=`prompt_nonempty_string 0 "entrez votre mot de passe"` ; then
      exit 1
    fi
    if ! PASSWORD1=`prompt_nonempty_string 0 "entrez à nouveau votre mot de passe"` ; then
      exit 1
    fi
...
setup_environment
show_info "Cet installateur a été préparé pour ${ORGANISATION}\n\nPlus d'informations et commentaires :\n\nEMAIL: ${SUPPORT}\nWWW: ${URL}\n\nCet installateur a été conçu en partenariat avec RENATER dans le cadre du projet pour le réseau européen de la recherche GÉANT (www.geant.net)"
if ! ask "Cet installateur ne fonctionnera correctement que si vous êtes membre de ${bf}Télécom SudParis & Institut Mines-Télécom Business School.${n}" "Continuer" 1 ; then exit; fi
if [ -d $HOME/.cat_installer ] ; then
   if ! ask "Le répertoire $HOME/.cat_installer existe. Certains des fichiers qu'il contient peuvent être écrasés." "Continuer" 1 ; then exit; fi
else
...
class EduroamNMConfigTool:
    ...
    def add_connection(self,ssid):
        server_alt_subject_name_list = dbus.Array({'DNS:radius.tem-tsp.eu'})
        server_name = 'radius.tem-tsp.eu'
        if self.nm_version == "0.9" or self.nm_version == "1.0":
        ...
        s_8021x = dbus.Dictionary({
            ...
            'anonymous-identity': 'anonymous@tem-tsp.eu',
        })
...
network={
  ...
  identity="${USER_NAME}"
  domain_suffix_match="radius.tem-tsp.eu"
  phase2="auth=MSCHAPV2"
  password="${PASSWORD}"
  anonymous_identity="anonymous@tem-tsp.eu"
}
EOFW
...
#prompt user for credentials
  ...
  if run_python_script ; then
   ...
   show_info "La configuration du gestionnaire de réseau a échoué. Génération du fichier wpa_supplicant.conf"
...
if [ -f $HOME/.cat_installer/cat_installer.conf ] ; then
  if ! ask "Le fichier $HOME/.cat_installer/cat_installer.conf existe. Il sera remplacé." "Continuer" 1 ; then confirm_exit; fi
  rm $HOME/.cat_installer/cat_installer.conf
  fi
   create_wpa_conf
   show_info "Sortie écrite dans $HOME/.cat_installer/cat_installer.conf"
fi
