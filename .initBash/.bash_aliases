...
alias connect2MyHiddenWifi="time nmcli con up id SebWlanFree && wanip"
...
alias gitUpdate="\grep -w url ./.git/config && \git pull"
alias gitURL="\grep -w url ./.git/config"
...
alias lprColor="\lpr -P $(lpstat -a 2>/dev/null | awk '/[Cc](olor|ouleur)/{prt=$1;exit}END{print prt}')"
alias a2psColor="\a2ps -P $(lpstat -a 2>/dev/null | awk '/[Cc](olor|ouleur)/{prt=$1;exit}END{print prt}')"
...
alias updatedb='updatedb -l 0 -o ~/.local/lib/mlocate/mlocate.db --prunefs "rpc_pipefs afs binfmt_misc proc smbfs autofs iso9660 ncpfs coda devpts ftpfs devfs mfs shfs sysfs cifs lustre tmpfs usbfs udf fuse.glusterfs fuse.sshfs curlftpfs"'
