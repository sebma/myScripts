...
thunderbirdVersion=$(thunderbird -V | awk '{print$NF}')
#xpiFile=$1
for xpiFile
do
	rm -f install.rdf
	...
	test $(id -u) = 0 && extensionDir=/usr/lib/thunderbird-$thunderbirdVersion/extensions || extensionDir=$HOME/.thunderbird/default/extensions
	echo "=> extensionDir = $extensionDir"
	 
	echo $thunderbirdVersion | grep -q "^3\." && {
