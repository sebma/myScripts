...
def main() :
	...
	with open( logFile , 'a' ) as logHandle :
		begin=datetime.now()
		if args.run == False : printNLogError( "=> WARNING: Starting the script < " + scriptBaseName + " > is running in SIMULATION mode on server " + platform.node() + ".\n" )
		else : printNLogError( "=> WARNING: Starting the script < " + scriptBaseName + " > is running in REAL mode on server " + platform.node() + ".\n" )

		printNLogString( "=> regExp = " + regExp )
		printNLogString( "=> Counting the files to be processed in the directory < " + args.dir + begin.strftime(' > at %X on the %d/%m/%Y') + " ..." )
		nbFiles = 0
		previous = 0.0
		...
		if totalNumberOfFiles :
			...
			if args.run :
				...
				for fileBaseName in matchingFileList :
						...
						try :
							...
							if args.overwrite :
								printNLogString( "=> INFO: Overwriting file < " + destination + os.sep + fileBaseName + " >." )
								remove( destination + os.sep + fileBaseName )
								move( fileBaseName, destination )
						else :
							nbFiles+=1
			...
			if args.run :
				printNLogString( "=> It took : " + str(end-begin) + " to move < " + str(nbFiles) +  " > files.\n" )
		else :
