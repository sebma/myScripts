...
def encode(source, dest, opts, vbr, abr=128, ac=2):
  command1 = ("ffmpeg -i \"%s\" -pass 1 -an -vcodec %s %s -b:v %d -bt %d -threads 0"\
    + " %s -y -f mp4 \"%s\"") % (source, vcodec, pass1preset, vbr, vbr, opts, p1out)
  command2 = ("ffmpeg -i \"%s\" -pass 2 -acodec %s -ac %d -ab %dk -vcodec %s %s "\
    + "-b:v %d -bt %d -threads 0 %s -y \"%s\"") \
    % (source, acodec, ac, abr, vcodec, pass2preset, vbr, vbr, opts, dest)
