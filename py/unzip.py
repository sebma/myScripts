...
def readZIPFile(zipFileName) :
	...
	try:
		...
		zipDirList = list(collections.OrderedDict.fromkeys(zipDirList)) #Pour supprimer les doublons
	...
	if   os.name == "posix" :
		if platform.system() == "Linux" :
			...
			if checkPythonVersion(2.7) :
				...
				outPut = check_output( "ps -elf | grep " + processName, shell=True )
				for line in outPut :
					print "line = " + line
			else :
				proc1 = Popen('ps -elf', stdout=PIPE)
				proc =  Popen('grep ' + processName, stdin=proc1.stdout, stdout=PIPE)
				for line in proc.stdout :
					print "line = " + line
			print >> stderr, "TO BE DONE !"
		...
		if isAdmin() :
			wmicProcessProcStdout, wmicProcessProcStderr = Popen("wmic process where \"name = '" + processName +"'\" get caption,commandline,processid -format:csv", stdout=PIPE, stderr=PIPE).communicate()
			...
			for line in tasklistProc.stdout :
				...
				if myProcessNamePattern.search( line ) and not tasklistPrompt.search( line ) :
...
def backupKillProcessNMoveFile_Complex(srcDir, dstRelativeFileName) :
	...
	if isRunning( baseFileName ) and isNewer( dstRelativeFileName, srcFileName ) :
		if args.kill :
			for pid in pidof_OLD( baseFileName ) :
				killProcess( pid )
			...
			printNLogError( "The process < " + baseFileName + " > of pid : " + " ".join( pidof_OLD( baseFileName ) ) + " is/are still running, cannot update it.\n" )
			nbErr += 1
	else :
...
def backupKillProcessNMoveFile(srcDir, dstRelativeFileName) :
	...
	baseFileName = basename( dstRelativeFileName )
	if args.overwrite or ( isNewer( dstRelativeFileName, srcFileName ) and isDifferent( dstRelativeFileName, srcFileName ) ) :
		pidList = getPIDsOfProcess( baseFileName )
		if len(pidList) :
			if args.kill :
				for pid in pidList :
					killProcess( pid )
			...
			if isDifferent( dstRelativeFileName, srcFileName ) :
				if not args.no_backup :
					backupFile(dstRelativeFileName)
				printNLogString( "  inflating: " + args.dir + "/" + dstRelativeFileName )
				move( srcFileName, dstRelativeFileName )
			else :
				printNLogErrorString( " => INFO : The file < " + args.dir + "/" + dstRelativeFileName + " > and the file < " + dstRelativeFileName + " > present in the archive are the same files." )
