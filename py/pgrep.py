			...
			for line in wmicProcessProc.stdout:
				...
				if myPattern.search( line ) :
					...
					if pid != selfPID :
						...
						p = Process(pid, process, command, line)
						Processes.append( p )
...
def main() :
	...
	class Process :
		def __init__(self, pid, name, commandLine, debugData) :
			self.pid = pid; self.name = name; self.commandLine = commandLine; self.debugData = debugData;
